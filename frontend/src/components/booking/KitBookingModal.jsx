import { useState, useEffect, useRef } from 'react'
import { createPortal } from 'react-dom'
import { useBooking } from '../../hooks/useBooking.js'

// ─── Helpers ──────────────────────────────────────────────────────────────────
function toLocalInput(date) {
  const pad = (n) => String(n).padStart(2, '0')
  return (
    `${date.getFullYear()}-${pad(date.getMonth() + 1)}-${pad(date.getDate())}` +
    `T${pad(date.getHours())}:${pad(date.getMinutes())}`
  )
}

// ─── KitBookingModal ──────────────────────────────────────────────────────────
// Props:
//   kit       — { id, name, item_count }
//   onClose   — called when modal should be dismissed
//   onSuccess — called after successful kit booking
// ──────────────────────────────────────────────────────────────────────────────
export default function KitBookingModal({ kit, onClose, onSuccess }) {
  const { bookKit, loading, error, success } = useBooking()
  const overlayRef = useRef(null)

  const defaultStart = () => {
    const d = new Date(Date.now() + 60 * 60 * 1000)
    d.setMinutes(d.getMinutes() < 30 ? 0 : 30, 0, 0)
    return d
  }
  const defaultEnd = () => {
    const d = defaultStart()
    d.setHours(d.getHours() + 1)
    return d
  }

  const [startInput, setStartInput] = useState(toLocalInput(defaultStart()))
  const [endInput,   setEndInput]   = useState(toLocalInput(defaultEnd()))
  const [notes,      setNotes]      = useState('')

  const startISO = startInput ? new Date(startInput).toISOString() : ''
  const endISO   = endInput   ? new Date(endInput).toISOString()   : ''

  // Close on Escape
  useEffect(() => {
    const onKey = (e) => { if (e.key === 'Escape') onClose() }
    document.addEventListener('keydown', onKey)
    return () => document.removeEventListener('keydown', onKey)
  }, [onClose])

  // Auto-dismiss after success
  useEffect(() => {
    if (success) {
      const t = setTimeout(() => { onSuccess?.(); onClose() }, 1800)
      return () => clearTimeout(t)
    }
  }, [success, onSuccess, onClose])

  const handleSubmit = async (e) => {
    e.preventDefault()
    await bookKit({ kit_id: kit.id, start_time: startISO, end_time: endISO, notes: notes || undefined })
  }

  const canBook = startISO && endISO && !loading

  const modal = (
    <div
      ref={overlayRef}
      className="fixed inset-0 z-50 flex items-center justify-center p-4"
      style={{ background: 'rgba(13,13,20,0.85)', backdropFilter: 'blur(6px)' }}
      onClick={(e) => { if (e.target === overlayRef.current) onClose() }}
    >
      <div
        id="kit-booking-modal"
        className="w-full max-w-lg glass-card overflow-hidden animate-slide-up"
        onClick={(e) => e.stopPropagation()}
      >
        {/* ── Header ── */}
        <div className="flex items-start justify-between p-6 pb-4 border-b border-surface-border">
          <div>
            <h2 className="text-base font-bold text-white">Book Kit</h2>
            <p className="text-sm text-white/45 mt-0.5 truncate max-w-xs">
              {kit.name}
              <span className="text-white/25 ml-2">· {kit.item_count} items</span>
            </p>
          </div>
          <button
            id="kit-booking-modal-close"
            onClick={onClose}
            className="btn-ghost px-2 py-1 text-white/40 hover:text-white ml-4 shrink-0"
          >
            ✕
          </button>
        </div>

        {/* ── Success state ── */}
        {success ? (
          <div className="p-8 text-center space-y-3">
            <div className="text-5xl animate-bounce">🎉</div>
            <p className="font-semibold text-white">Kit booked successfully!</p>
            <p className="text-sm text-white/40">All {kit.item_count} items reserved. Closing…</p>
          </div>
        ) : (
          <form id="kit-booking-modal-form" onSubmit={handleSubmit} className="p-6 space-y-5">

            {/* Atomic note */}
            <div className="rounded-xl border border-accent-cyan/20 bg-accent-cyan/5 px-4 py-3">
              <p className="text-xs text-accent-cyan/80 font-semibold mb-1">Atomic Kit Booking</p>
              <p className="text-[11px] text-white/35 leading-relaxed">
                All {kit.item_count} items are checked and reserved together. If any single item
                has a conflict, the entire booking is rejected — no partial bookings.
              </p>
            </div>

            {/* Date range */}
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                <label htmlFor="kit-booking-start" className="text-xs font-semibold text-white/50 uppercase tracking-wider">
                  From
                </label>
                <input
                  id="kit-booking-start"
                  type="datetime-local"
                  required
                  value={startInput}
                  onChange={(e) => setStartInput(e.target.value)}
                  className="input-field text-sm"
                />
              </div>
              <div className="space-y-1.5">
                <label htmlFor="kit-booking-end" className="text-xs font-semibold text-white/50 uppercase tracking-wider">
                  To
                </label>
                <input
                  id="kit-booking-end"
                  type="datetime-local"
                  required
                  value={endInput}
                  onChange={(e) => setEndInput(e.target.value)}
                  className="input-field text-sm"
                />
              </div>
            </div>

            {/* Notes */}
            <div className="space-y-1.5">
              <label htmlFor="kit-booking-notes" className="text-xs font-semibold text-white/50 uppercase tracking-wider">
                Notes <span className="text-white/25 font-normal normal-case tracking-normal">(optional)</span>
              </label>
              <textarea
                id="kit-booking-notes"
                rows={2}
                maxLength={1000}
                value={notes}
                onChange={(e) => setNotes(e.target.value)}
                placeholder="Project name, shoot details…"
                className="input-field resize-none"
              />
            </div>

            {/* Conflict details from API (409 error) */}
            {error && (
              <div className="rounded-xl border border-accent-rose/30 bg-accent-rose/5 px-4 py-3 space-y-2">
                <p className="text-sm font-semibold text-accent-rose">⚠ Kit booking failed</p>
                <p className="text-xs text-accent-rose/70">{error.message}</p>
              </div>
            )}

            {/* Actions */}
            <div className="flex gap-3 pt-1">
              <button type="button" onClick={onClose} className="btn-secondary flex-1 justify-center">
                Cancel
              </button>
              <button
                id="kit-booking-modal-submit"
                type="submit"
                disabled={!canBook}
                className="btn-primary flex-1 justify-center disabled:opacity-40 disabled:cursor-not-allowed disabled:shadow-none"
              >
                {loading ? (
                  <>
                    <span className="w-4 h-4 rounded-full border-2 border-white/30 border-t-white animate-spin" />
                    Booking Kit…
                  </>
                ) : (
                  <>
                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M20.25 7.5l-.625 10.632a2.25 2.25 0 01-2.247 2.118H6.622a2.25 2.25 0 01-2.247-2.118L3.75 7.5M10 11.25h4M3.375 7.5h17.25c.621 0 1.125-.504 1.125-1.125v-1.5c0-.621-.504-1.125-1.125-1.125H3.375c-.621 0-1.125.504-1.125 1.125v1.5c0 .621.504 1.125 1.125 1.125z" />
                    </svg>
                    Book All {kit.item_count} Items
                  </>
                )}
              </button>
            </div>
          </form>
        )}
      </div>
    </div>
  )

  return createPortal(modal, document.body)
}
